                                     ...
                                     dw::core::style::TextAlignType text_align)
{
  ...
  case DILLO_HTML_TABLE_MODE_TD:
    ...
    if (a_Html_get_attr(html, tag, tagsize, "nowrap")) {
      ...
      html->styleEngine->setNonCssHint(CSS_PROPERTY_WHITE_SPACE, CSS_TYPE_ENUM,
                                       WHITE_SPACE_NOWRAP);
    }
                                     ...
                                     const char *tag, int tagsize)
{
  ...
  case DILLO_HTML_TABLE_MODE_TD:
    ...
    if (html->style()->textAlign == TEXT_ALIGN_STRING)
      col_tb =
          new dw::TableCell(((dw::Table *)S_TOP(html)->table)->getCellRef(),
