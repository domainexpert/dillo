...
void DilloHtml::bugMessage(const char *format, ... )
{
  ...
  dStr_sprintfa(bw->page_bugs, "HTML warning: line %d, ", getCurrLineNumber());
                         ...
                         int use_base_url)
{
  ...
  url =
      a_Url_new(url_str, (use_base_url) ? base_url : URL_STR_(html->base_url));
  if ((n_ic = URL_ILLEGAL_CHARS(url)) != 0) {
    ...
    if (n_ic == n_ic_spc) {
      ...
      BUG_MSG("URL has %d illegal byte%s in {00-1F, 7F-FF} range ('%s').", n_ic,
              suffix, url_str);
    } else {
      BUG_MSG("URL has %d illegal byte%s: "
              "%d space%s and %d in {00-1F, 7F-FF} range ('%s').",
              n_ic, suffix, n_ic_spc, n_ic_spc > 1 ? "s" : "", n_ic - n_ic_spc,
              url_str);
    }
   }
   return url;
...
bool a_Html_tag_set_valign_attr(DilloHtml *html, const char *tag, int tagsize)
{
  ...
  if ((attr = a_Html_get_attr(html, tag, tagsize, "valign"))) {
    ...
    html->styleEngine->setNonCssHint(CSS_PROPERTY_VERTICAL_ALIGN, CSS_TYPE_ENUM,
                                     valign);
    return true;
   } else
      return false;
                       ...
                       DilloHtmlEnc enc, const char *charset)
{
  ...
  DilloHtmlForm *form =
      a_Html_form_new(this, method, action, enc, charset, enabled);
  int nf = forms->size();
  forms->increase();
  forms->set(nf, form);
...
static int Html_entity_search(char *key)
{
  ...
  EntKey.entity = key;
  res = (Ent_t *)bsearch(&EntKey, Entities, NumEnt, sizeof(Ent_t),
                         Html_entity_comp);
  if (res)
    return (res - Entities);
  return -1;
...
char *a_Html_parse_entities(DilloHtml *html, const char *token, int toksize)
{
  ...
  for (i = j = s; i < toksize; i++) {
    if (token[i] == '&' &&
        (isocode = Html_parse_entity(html, token + i, toksize - i, &entsize)) >=
            0) {
      if (isocode >= 128) {
        /* multibyte encoding */
...
static void Html_tag_cleanup_nested_inputs(DilloHtml *html, int new_idx)
{
  ...
  u_idx = (html->InFlags & IN_BUTTON) ? i_BUTTON : (html->InFlags & IN_SELECT)
                                                       ? i_SELECT
                                                       : i_TEXTAREA;
...
static void Html_tag_open_h(DilloHtml *html, const char *tag, int tagsize)
{
  ...
  a_Html_stash_init(html);
  S_TOP(html)->parse_mode = DILLO_HTML_PARSE_MODE_STASH_AND_BODY;
}
...
static const char* Html_get_javascript_link(DilloHtml *html)
{
  ...
  if (dStrnAsciiCasecmp("javascript", Buf->str, 10) == 0) {
    i = strcspn(Buf->str, "'\"");
    ch = Buf->str[i];
    if ((ch == '"' || ch == '\'') && (p2 = strchr(Buf->str + i + 1, ch))) {
      p1 = Buf->str + i;
...
static int Html_tag_pre_excludes(DilloHtml *html, int tag_idx)
{
  ...
  if (!(html->DocType == DT_HTML && html->DocTypeVersion >= 5.0f)) {
    ...
    const char *es_set[] = {"img", "object", "applet", "big",      "small",
                            "sub", "sup",    "font",   "basefont", NULL};
...
static void Html_update_content_type(DilloHtml *html, const char *content)
{
  ...
  const char *new_content =
      a_Capi_set_content_type(html->page_url, content, "meta");
...
static void Html_tag_open_meta(DilloHtml *html, const char *tag, int tagsize)
{
  ...
  const char meta_template[] =
      ...
      " This page uses the NON-STANDARD meta refresh tag.<br> The HTML 4.01 "
      "SPEC\n"
  ...
  if (html->InFlags & IN_HEAD) {
    if ((attrbuf = a_Html_get_attr(html, tag, tagsize, "href"))) {
      ...
      if (URL_SCHEME_(BaseUrl)) {
        ...
        a_Url_set_flags(BaseUrl, URL_FLAGS(html->base_url) & URL_SpamSafe);
  ...
  while (html->stack->size() > 1) {
    ...
    if (Tags[oldtag_idx].EndTag == 'R') {
      BUG_MSG("<%s> is not allowed to contain <%s>. -- closing <%s>.",
              Tags[oldtag_idx].name, Tags[new_idx].name, Tags[oldtag_idx].name);
    }

    /* Workaround for Apache and its bad HTML directory listings... */
    if ((html->InFlags & IN_PRE) && strcmp(Tags[new_idx].name, "hr") == 0)
      break;
  ...
  for (i = 1; i < tagsize; ++i) {
    ...
    case GET_VALUE:
      if ((delimiter == ' ' && (isspace(tag[i]) || tag[i] == '>')) ||
          tag[i] == delimiter) {
        state = FINISHED;
      } else if (tag[i] == '&' && (tag_parsing_flags & HTML_ParseEntities)) {
        if ((isocode = Html_parse_entity(html, tag + i, tagsize - i,
                                         &entsize)) >= 0) {
          if (isocode >= 128) {
            char buf[4];
