...
Png_datainfo_callback(png_structp png_ptr, png_infop info_ptr)
{
  ...
  png_get_IHDR(png_ptr, info_ptr, &png->width, &png->height, &bit_depth,
...
static void Png_write(DilloPng *png, void *Buf, uint_t BufSize)
{
  ...
  while (png->state != IS_finished && DATASIZE) {
    ...
    case IS_init:
      ...
      png->png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, png,
    ...
    case IS_nextdata:
      if (setjmp(png->jmpbuf)) {
        ...
        png_process_data(png->png_ptr, png->info_ptr,
                         png->ipbuf + png->ipbufstart, (png_size_t)DATASIZE);
